...
def _redact_sensitive_text(s: str) -> str:
    """
    Best-effort redaction for identity-bearing absolute paths in logs.
    """
...
def count_jsonl(path_str: str) -> int:
    ...
    path = Path(path_str)
    if not path.exists():
        raise FileNotFoundError(f"JSONL file not found: {_redact_sensitive_text(str(path))}")
    n = 0
...
def run_mp_main(mp_main: Path, config_path: Path):
    print(f"[RUN] MP_main.py with config = {_redact_sensitive_text(str(config_path))}")
    subprocess.run(
...
def build_retrieval_config(
    ...
    """
    return {
        "data": {"train_data_path": train_path, "test_data_path": test_path},
        "influence": {
            "outdir": outdir,
            "seed": seed,
            "cal_words_infl": False,
...
def rename_influence_file(outdir: Path, n_train: int, suffix: str) -> Path:
    """
    ...
    if not candidates:
        raise FileNotFoundError(
            "No influence result files found in "
            f"{_redact_sensitive_text(str(outdir))} matching pattern {pattern}"
        )
...
def main():
    ...
    # ===== 0) YAML config =====
    parser.add_argument("--config", type=str, default=None, help="YAML config file for the RapidIn pipeline")
    ...
    # IMPORTANT: previously this was `action="store_true", default=True` which makes it impossible
    # to explicitly pass `False` via CLI. Keep behavior but add an opt-out flag.
    ...
    parser.add_argument("--top_k", type=int, default=1000, help="RapidIn influence top_k (per test)")
    ...
    if raw_args.config:
        ...
        if not cfg_path.exists():
            raise FileNotFoundError(f"Config YAML not found: {_redact_sensitive_text(str(cfg_path))}")
    ...
    if not mp_main.exists():
        raise FileNotFoundError(f"MP_main.py not found at: {_redact_sensitive_text(str(mp_main))}")

    # Infer n_forget / n_retain if not provided
    n_forget = count_jsonl(args.forget_path) if args.n_forget is None else args.n_forget
    n_retain = count_jsonl(args.retain_path) if args.n_retain is None else args.n_retain
    ...
    if not mapping_script.exists():
        raise FileNotFoundError(f"Mapping script not found: {_redact_sensitive_text(str(mapping_script))}")
    ...
    cmd = [
        ...
        str(args.wmax_r),
        "--out_forget",
        str(Path(args.out_forget).resolve()),
        "--out_retain",
        str(Path(args.out_retain).resolve()),
    ]

    subprocess.run(cmd, check=True)

    print("[DONE] RapidIn pipeline finished.")
    print(f"       Forget weights: {_redact_sensitive_text(str(Path(args.out_forget).resolve()))}")
    print(f"       Retain weights: {_redact_sensitive_text(str(Path(args.out_retain).resolve()))}")
    ...
    if args.cleanup_intermediate:
        ...
        for d in [args.forget_grads, args.retain_grads]:
            p = Path(d).resolve()
            if p.exists():
                print(f"  - rm -r {_redact_sensitive_text(str(p))}")
